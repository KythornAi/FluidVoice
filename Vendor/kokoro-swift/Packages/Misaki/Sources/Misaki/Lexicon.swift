...
public final class Lexicon {
    ...
    func getWord(_ word: String, tag: String?, stress: Double?, ctx: TokenContext) -> (String?, Int?) {
        ...
        // FluidChat fix: unknown proper nouns (character names in ebooks)
        // vanished silently because getWord never reached the NNP
        // letter-spelling fallback for words of 7 letters or fewer.
        // Spell them out like upstream Misaki does.
        if tag == "NNP", !lookupWord.isEmpty, lookupWord.allSatisfy(\.isLetter) {
            let nnpResult = getNNP(lookupWord)
            if nnpResult.0 != nil { return nnpResult }
        }
